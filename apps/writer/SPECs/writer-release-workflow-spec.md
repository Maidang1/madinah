...
- GitHub Actions uses ad-hoc macOS signing and delegates the build, updater artifacts,
  and draft GitHub Release to `tauri-action`; no Apple certificate or notarization
  account is required.
...
- A valid tag produces an ad-hoc-signed DMG, updater archive, updater signature, and
  `latest.json` in one draft GitHub Release.
