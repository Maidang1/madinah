...
author: Madinah
description: Mini-Kode 架构学习笔记，记录 CLI 启动、Agent 执行循环、工具调用、权限系统和错误处理。
tags:
  - AI
  - Agent
...
## 系统架构概览[link text](https://)

Mini-Kode 是一个基于 LLM 的命令行编程助手，通过 **工具调用（Tool Calling）** 和大语言模型交互。整体结构可以拆成三层：CLI UI、Agent 执行引擎、工具与权限系统。
...
Agent 执行引擎是系统核心，负责维护 LLM 和工具之间的循环：发送消息、接收流式响应、执行工具调用，再把工具结果写回对话。
...
Mini-Kode 实现了两层权限系统：一层是运行模式，另一层是权限策略。
...
OpenAI API 对工具消息顺序有严格要求，顺序错误会导致 400 响应。

**关键规则：**

1. **Tool 消息必须跟在包含 tool\_calls 的 Assistant 消息后面**
   ```typescript
   // ✅ 正确
   [
     { role: "user", content: "..." },
   ...
   ```
2. **每个 tool\_call 必须有且仅有一个对应的 tool 消息**
3. **Tool 消息必须按照 tool\_calls 数组的顺序**
   ```typescript
   // ✅ 正确
   [
...
3. **LLM 循环**：
   ...
   * 流式接收响应
   * 判断 finish\_reason
   * 如果是 tool\_calls：执行工具 → 添加结果 → 继续循环
   * 如果是 stop：返回最终响应
4. **工具执行**：
   * 只读工具并发执行
   * 写入工具顺序执行
...
* **类型安全**：TypeScript 和 Zod 分别覆盖编译期与运行时校验。
* **执行效率**：只读工具可以并发执行。
* **权限可控**：文件、命令和 MCP 工具都有独立授权路径。
* **易于扩展**：新工具可以按统一接口注册。
* **反馈及时**：流式响应让用户能看到任务进度。
